  ...
  try {
    ...
    if (!process.env.RAZORPAY_TEST_KEY_ID || !process.env.RAZORPAY_TEST_KEY_SECRET) {
      ...
      return res.status(500).json({ 
        msg: "Server configuration error: Payment keys missing",
        error: { code: 'CONFIG_ERROR', description: 'Razorpay keys not configured' }
      });
    }
    ...
    console.error('Error creating donation order:', err);
    res.status(500).json({ 
      msg: "Server Error creating donation order", 
      error: err.message,
      details: err.stack
    });
  }
...
router.post("/verify", async (req, res) => {
  ...
  try {
    if (!process.env.RAZORPAY_TEST_KEY_SECRET) {
      console.error("RAZORPAY_TEST_KEY_SECRET is not defined in environment variables");
      return res.status(500).json({ msg: "Server configuration error: Payment secret key missing" });
    }
